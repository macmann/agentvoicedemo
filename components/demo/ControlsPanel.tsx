...
export function ControlsPanel(props: Props) {
  return (
    <section className="rounded-xl border border-slate-200 bg-white p-4">
      ...
      <p className="mt-1 text-xs text-slate-500">Use these toggles to exercise success, fallback, clarify, and handoff paths without changing orchestration logic.</p>
      <div className="mt-3 space-y-3 text-sm">
        ...
        <div className="flex flex-wrap gap-2 pt-1">
          <button className="rounded bg-indigo-600 px-3 py-2 text-white" onClick={props.onRun} type="button">
            {props.stepMode ? "Run current step" : "Run full simulation"}
          </button>
          <button className="rounded border border-slate-300 px-3 py-2 disabled:bg-slate-100" onClick={props.onNext} disabled={props.nextDisabled} type="button">
            Next step
          </button>
