...
export function parseScenarioSignals(utterance: string): ScenarioSignals {
  ...
  if (reschedule) {
    return {
      ...
      supportIntent: true,
      emotionOnly: false,
      empathyNeeded: discomfortDetected || frustration,
      discomfortDetected,
      explicitHumanRequest,
      sentiment: discomfortDetected || frustration ? "negative" : "neutral",
      confidence: 0.9,
      entities: { issueType: "appointment", action: "reschedule" },
      sttFailureHint
    };
  }
  ...
  if (routerIssue) {
    return {
      ...
      sentiment: frustration ? "negative" : "neutral",
      confidence: 0.64,
      entities: { issueType: "connectivity", symptom: "router_blinking_red" },
      sttFailureHint
    };
  }
  ...
  if (discomfortDetected || frustration) {
    return {
      ...
      sentiment: "negative",
      confidence: 0.88,
      entities: { context: discomfortDetected ? "health_discomfort" : "frustration" },
      sttFailureHint
    };
  }
  ...
  return {
    ...
    sentiment: "neutral",
    confidence: 0.58,
    entities: { issueType: "general_support" },
    sttFailureHint
  };
}
